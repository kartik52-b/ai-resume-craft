...
const HealthcareTemplate = ({ data }: { data: ResumeData }) => {
    ...
    switch (id) {
      case 'experience': return experience.length > 0 && (
        <div className="mb-3 break-inside-avoid">
          <h2 className="text-[11px] font-bold uppercase text-sky-800 border-b-2 border-sky-600 pb-0.5 mb-1.5">Clinical Experience</h2>
          {experience.map(exp => (
            <div key={exp.id} className="mb-2.5 last:mb-0">
              ...
              <div className="text-[10px] text-sky-700 font-medium">{exp.company}{exp.location ? `, ${exp.location}` : ''}</div>
              {exp.bullets.filter(Boolean).length > 0 && <ul className="mt-1 space-y-0.5 text-[10.5px] text-gray-600 list-disc list-outside ml-3.5">{exp.bullets.filter(Boolean).map((b, i) => <li key={i}>{b}</li>)}</ul>}
            </div>
          ))}
        </div>
      );
      case 'education': return education.length > 0 && (
        <div className="mb-3">
          <h2 className="text-[11px] font-bold uppercase text-sky-800 border-b-2 border-sky-600 pb-0.5 mb-1.5">Education</h2>
      ...
      case 'skills': return skills.length > 0 && (
        <div className="mb-3">
          <h2 className="text-[11px] font-bold uppercase text-sky-800 border-b-2 border-sky-600 pb-0.5 mb-1.5">Clinical Skills</h2>
          <div className="grid grid-cols-2 gap-0.5 text-[10.5px] text-gray-600">{skills.map((s, i) => <span key={i}>• {s}</span>)}</div>
        </div>
      );
      case 'certifications': return certifications.length > 0 && (
        <div className="mb-3">
          <h2 className="text-[11px] font-bold uppercase text-sky-800 border-b-2 border-sky-600 pb-0.5 mb-1.5">Licenses & Certifications</h2>
      ...
      case 'projects': return projects.length > 0 && (
        <div className="mb-3 break-inside-avoid">
          <h2 className="text-[11px] font-bold uppercase text-sky-800 border-b-2 border-sky-600 pb-0.5 mb-1.5">Research & Projects</h2>
          {projects.map(proj => <div key={proj.id} className="mb-2 last:mb-0"><span className="font-bold text-gray-900 text-[11px]">{proj.name}</span>{proj.description && <p className="text-[10.5px] text-gray-600 mt-0.5">{proj.description}</p>}</div>)}
        </div>
      );
      default: return null;
    }
  ...
  return (
    <div className="font-resume-sans text-[10.5px] leading-[1.5] text-gray-800">
      <div className="mb-4 pb-3 border-b-2 border-sky-600">
        {p.fullName && <h1 className="text-xl font-bold text-gray-900">{p.fullName}</h1>}
        {p.headline && <div className="text-[10.5px] font-semibold text-sky-700 mt-0.5">{p.headline}</div>}
